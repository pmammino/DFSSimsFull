...
### Done — team wins, save/hold context, free agents

| Item | Where | Effect |
|---|---|---|
| **Team runs allowed** | `team_wins.bottom_up_team_ra` | Defensive mirror of the offensive factor, same linear weights, so both sides sit on one scale. |
| **Team wins** | `team_wins.project_team_wins` | Pythagenpat, normalized on the odds scale so league win% averages exactly .500 and wins total exactly **2,430** (was 2,618). Fixes the RS ≠ RA identity (4.28 vs 3.86). |
| **Save / hold pools** | `team_wins.project_save_hold_opportunity` | Team opportunity scaled off wins with a sub-linear elasticity (a save needs a *narrow* lead; blowouts produce none), normalized to league totals so elasticity changes distribution only. |
| **Free agents** | `FREE_AGENT_TEAM_ID`, `mlb_clubs_only` | Distinct from "team unknown". FAs keep full rate lines and are assignable playing time, but are excluded from team aggregates so they can't form a 31st club. |
| **Roster reserves** | `load_roster_reserves` | A club can hold back part of its PA/IP budget for a signing it hasn't made, so it is deliberately under-projected instead of over-projecting the incumbents. |

**Wins did not need the playing-time model.** Counting stats need playing time;
rates do not. The audit's 2,618 wins came from offense and defense being
aggregated over differently-selected pools, not from missing playing time.

`SAVES_PER_WIN`, `HOLDS_PER_WIN` and the elasticities are documented starting
points, not fitted values — the weakest numbers in the module.
`fit_opportunity_rates` replaces them from real team-seasons once
`fetch_team_rpg` carries team SV/HLD.

### Planned, with inputs generated — roles

The taxonomy is designed in README_projection_engine.md and its input workbooks
are generated by `scripts/build_role_templates.py`
(`rosters/player_roles_{hitters,pitchers}_2027.xlsx`). The **model is not
built**. Key design calls: role as a probability vector rather than a label; the
job separated from the timing (which unifies callups with return-from-injury);
availability as an orthogonal multiplier; catchers on their own ladder; and
role-conditional `vL_share`, which fixes a live bug where `splits_model.py:153`
derives platoon exposure from a player's *past* usage.
